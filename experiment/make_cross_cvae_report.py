...
def _grasp_losses_per_hand(validate_payload: dict[str, Any]) -> dict[str, dict[str, float | None]]:
    per_hand_joint = {}
    per_hand_chamfer = {}
    ...
    all_hands = sorted(set(per_hand_joint.keys()) | set(per_hand_chamfer.keys()))
...
def _table_md(title: str, rows: dict[str, dict[str, float | None]]) -> str:
    lines = [f"## {title}", ""]
    lines.append("| hand | grasp_joint_l1 | grasp_chamfer | GT-in-hand-rate | in-hand-rate | in-hand-rate all |")
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: |")
    for hand in sorted(rows.keys()):
        ...
        lines.append(
            ...
            f"{_to_text(r.get('in_hand_rate'))} | {_to_text(r.get('in_hand_rate_all'))} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    train_rows = _merge_metrics(
        _grasp_losses_per_hand(train_validate),
    ...
    test_rows = _merge_metrics(
        _grasp_losses_per_hand(test_validate),
