    ...
    def __init__(
        ...
        *,
        ...
        if action_normalizer_scale.shape != action_normalizer_offset.shape:
            raise ValueError(
                f"action normalizer scale/offset shape mismatch: "
        ...
        if action_normalizer_scale.ndim != 2 or int(action_normalizer_scale.shape[1]) != self.action_dim:
            raise ValueError(
                f"action normalizer must have shape (num_robots, {self.action_dim}), "
                f"got {tuple(action_normalizer_scale.shape)}"
            )
