...
class DGCNNObjectEncoder(nn.Module):
    """Small DGCNN encoder that maps object point clouds to a global feature."""
    ...
    def forward(self, object_pc: torch.Tensor) -> torch.Tensor:
        # object_pc: (B, N, 3)
        x = object_pc.transpose(1, 2).contiguous()  # (B, 3, N)
        ...
        x = self.conv5(x)
        return F.adaptive_max_pool1d(x, 1).squeeze(-1)  # (B, emb_dim)


class CrossEmbodimentActionCVAE(nn.Module):
    """Basic CVAE: [object pc + embodiment] conditions both encoder and decoder."""
    ...
    def encode(
        ...
        action: torch.Tensor,
        embodiment_idx: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        object_feat = self.object_encoder(object_pc)
        emb_feat = self.embodiment_embedding(embodiment_idx)
        fused = torch.cat([action, object_feat, emb_feat], dim=-1)
        latent_stats = self.encoder(fused)
    ...
    def decode(
        ...
        object_feat = self.object_encoder(object_pc)
        emb_feat = self.embodiment_embedding(embodiment_idx)
        fused = torch.cat([z, object_feat, emb_feat], dim=-1)
    ...
    def forward(
        ...
        recon = self.decoder(torch.cat([z, object_feat, emb_feat], dim=-1))
